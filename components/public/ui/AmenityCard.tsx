...
export default function AmenityCard({ name, image, href = '#', aspect = 'portrait' }: AmenityCardProps) {
  ...
  return (
    <Link
      href={href}
      className={`group relative block ${aspectClass} min-w-[200px] flex-shrink-0 overflow-hidden rounded-xl bg-tertiary md:min-w-[250px]`}
    >
      <Image
        src={image}
        alt={name}
        ...
        sizes={aspect === 'portrait' ? '250px' : '(max-width: 768px) 100vw, 33vw'}
      />
